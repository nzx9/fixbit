...
function IssueEditDialog(props) {
  ...
  const [title, setTitle] = React.useState(props.issueInfo.title);
  const [description, setDescription] = React.useState(
    props.issueInfo.description
  );
  const [isOpen, setIsOpen] = React.useState(
    props.issueInfo.is_open === 1 ? true : false
  );
  const [assignTo, setAssignTo] = React.useState(
    props.issueInfo.assign_to === null ? -1 : props.issueInfo.assign_to
  );
  const [priority, setPriority] = React.useState(props.issueInfo.priority);
  const [type, setType] = React.useState(props.issueInfo.type);
      ...
      >
        <form
          ...
          onSubmit={(e) => {
            ...
            httpReq(
              `${config.URL}/api/projects/${props.pid}/issues/${props.iid}`,
              "PUT",
              data,
              token
            )
              ...
              <Grid item xs={12} md={6}>
                ...
                >
                  ...
                  >
                    ...
                    {props.teamInfo !== null &&
                    props.teamInfo?.members !== null ? (
                      props.teamInfo.members.map((value, index) =>
                        value.uid === uId ? (
                          <MenuItem key={index} value={value.uid}>
                            {value.name}&nbsp;<b>(Me)</b>
                          </MenuItem>
                        ) : value.uid === props.adminInfo.id ? (
                          <MenuItem key={index} value={value.uid}>
                    ...
                    ) : props.adminInfo.id === uId ? (
                      <MenuItem value={props.adminInfo.id}>
                        {props.adminInfo.username}&nbsp;<b>(Me)</b>
                      </MenuItem>
                    ) : (
                      <MenuItem value={props.adminInfo.id}>
                        {props.adminInfo.username}&nbsp;<b>(Admin)</b>
                      </MenuItem>
                    )}
                  </Select>
                </FormControl>
